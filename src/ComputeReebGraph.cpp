...
int LoadData(_SimpleMesh &mesh, std::vector<Vector3> &meshNormal,
	const char* mesh_file_name, double &BoundingBoxRadius, std::vector<int> &tris, std::set<int> &extraVertices, int &orgTriangleSize,
	const float fEnlargeFactor)
{
	int genus = 0;
	_SimpleMeshVertex minBd;
	_SimpleMeshVertex maxBd;
	mesh.LoadMeshInOFFformat(minBd, maxBd, meshNormal, mesh_file_name, tris, fEnlargeFactor);//"E:\\RProgramming\\Models\\torus.off");// "D:\\MeshModels\\OFF-models\\HighGenusCubeHC1.off");//
	...
	if (CheckBoundaries(mesh, meshBoundaries))
...
int main(int argc, char **argv)
{
	...
	if (ParseCommand(argc, argv, InputFileName, OutputFileName))
	{
		...
		LoadData(mesh, meshNormal, InputFileName.c_str(), BoundingBoxRadius, OrientTriangles, extraVertices, nOrgTriangleSize, fEnlargeFactor);
		//////
		...
		//{
		//	//reebGraph.WriteReebGraph("casting.reeb");
		...
		if (extraVertices.empty())
			CycleLocalOptimization(mesh, reebGraph, OrientTriangles, v_basis_loops, h_basis_loops, 1.f / fEnlargeFactor);
		else
			CycleLocalOptimization_bdry(mesh, reebGraph, OrientTriangles, v_basis_loops, h_basis_loops, extraVertices, 1.f / fEnlargeFactor);
		//
