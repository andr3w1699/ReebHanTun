...
void LoadCycleData(psbmReebGraph &reebGraph, std::vector<std::vector<int> > &basis_h_loops, std::vector<std::vector<int> > &basis_v_loops, std::vector<int> &base_pt_vec)
{
	//
	...
	{
		if ((*reebGraph.initVerticalLoops)[i].pathType)
		{// this is a vertical loop
	...
	for ( int i = 0; i < tot_genus; i++)
	{
			...
			listIter != (reebGraph.invLinkNumbersMatrix)[row_num].end(); listIter++)
		{
			...
			if (col_num >= tot_genus)
			{// it is a horizontal loop
				edges_num += counter * (int)(reebGraph.vecHorizontalLoopEdgePathOnMesh_Edge)[col_num - tot_genus].size();
			}
			else
			{
			...
			listIter != (reebGraph.invLinkNumbersMatrix)[row_num].end(); listIter++)
		{
			...
			{// it is a horizontal loop
				...
				{
					for (std::vector<int>::iterator vIter = (reebGraph.vecHorizontalLoopEdgePathOnMesh_Edge)[col_num - tot_genus].begin();
						vIter != (reebGraph.vecHorizontalLoopEdgePathOnMesh_Edge)[col_num - tot_genus].end(); vIter++)
					{
						//sstr << *vIter << " ";
...
void AssembleMeshGraph(_simpGraph_vec & meshGraph, _SimpleMesh &mesh, std::vector<float> & sqEdgeLength, std::set<int> &extraVertices, const float fScaleRatio)
{
...
void AssembleMeshGraph(_simpGraph_vec & meshGraph, _SimpleMesh &mesh, std::vector<float> & sqEdgeLength, const float fScaleRatio)
{
	meshGraph.InitNodes((int)mesh.vecVertex.size());
	//
	for (unsigned int eid = 0; eid < mesh.vecEdge.size(); eid++)
...
//	//edge_anno_proxy.ReadEdgeAnnotationsFromFile(argv[2]);
	...
	edge_anno_proxy.CheckTwoGroupVectorOrthogonality(basis_v_loops, basis_v_annotations, basis_h_loops, basis_h_annotations);
	//
...
//		boost::progress_timer t;
		...
		do
		{
			...
			base_pt_vec.clear();
			//
			cano_loop_proxy.compute_canonical_loops_fibo(cano_loops, non_tree_edges, cano_loop_annotations, sorted_cano_loops, edge_anno_proxy);
			//
			if (edge_anno_proxy.ComputeShortestCanonicalLoops(basis_h_loops, basis_h_annotations, cano_loops, non_tree_edges,
				cano_loop_annotations, sorted_cano_loops, tmp_short_h_basis_loop, non_tree_edges_for_sorted_cano_loops))
			{
			...
			if (edge_anno_proxy.ComputeShortestCanonicalLoops(basis_v_loops, basis_v_annotations, cano_loops, non_tree_edges,
				cano_loop_annotations, sorted_cano_loops, tmp_short_v_basis_loop,non_tree_edges_for_sorted_cano_loops))
			{
				std::cout << "."; //ompletly use short loops" << std::endl;
			}
			else
...
{//
	...
	edge_anno_proxy.CheckTwoGroupVectorOrthogonality(basis_v_loops, basis_v_annotations, reduced_basis_v_annotations, basis_v_LowestOnePos,
		basis_h_loops, basis_h_annotations, reduced_basis_h_annotations, basis_h_LowestOnePos);
	//
	canonical_loops_computing cano_loop_proxy;
	//
	//
	...
	std::vector<std::set<int> > tmp_short_v_basis_loop;
	{
		...
		do
		{
			...
			if (edge_anno_proxy.ComputeShortestCanonicalLoops(basis_h_loops, basis_h_annotations, reduced_basis_h_annotations,  basis_h_LowestOnePos, cano_loops, non_tree_edges,
				cano_loop_annotations, sorted_cano_loops, tmp_short_h_basis_loop, non_tree_edges_for_sorted_cano_loops))
			{
				std::cout << ".";//ompletly use short loops" << std::endl;
			}
			...
			if (edge_anno_proxy.ComputeShortestCanonicalLoops(basis_v_loops, basis_v_annotations, reduced_basis_v_annotations, basis_v_LowestOnePos,
		...
		}while (fabs(cur_basis_h_len - pre_basis_h_len) > 0.000001 ||  fabs(cur_basis_v_len - pre_basis_v_len) > 0.000001);
		//
		std::cout << std::endl;
	}
	//
